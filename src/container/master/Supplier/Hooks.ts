...
export const SupplierHooks = () => {
  ...
  const AddSupplierFormik = useFormik({
    ...
    initialValues: {
      supplierName:
        editData && Object.keys(editData).length > 0 ? editData.Supp_Name : "",
      supplierAddress:
        editData && Object.keys(editData).length > 0 ? editData.Supp_Add : "",
      supplierEmail:
        editData && Object.keys(editData).length > 0 ? editData.Supp_Mail : "",
      gstin:
        editData && Object.keys(editData).length > 0 ? editData.Supp_GSTIN : "",
      maxCredDays:
        editData && Object.keys(editData).length > 0 ? editData.Max_Days : 0,
      openingBalance:
        editData && Object.keys(editData).length > 0 ? editData.Open_Bal : 0,
      underLedger:
        editData && Object.keys(editData).length > 0 ? editData.Link_Gl : "",
    },
    validationSchema: Yup.object().shape({
      supplierName: Yup.string().required(
        text.errors.requiredErrors.supplier.name
      ),
      supplierAddress: Yup.string().required(
        text.errors.requiredErrors.supplier.address
      ),
      supplierEmail: Yup.string().matches(
        EMAIL_REGEX,
        text.errors.patternErrors.supplier.email
      ),
      gstin: Yup.string(),
      ...
      underLedger: Yup.string().required(
        text.errors.requiredErrors.supplier.underLedger
      ),
    }),
    onSubmit: (values) => {
  ...
  const editSupplierApiCall = async (
    supplierId: number,
    orgId: number,
    item: any
  ) => {
